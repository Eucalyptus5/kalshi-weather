...
SENTINELS_0010: tuple[tuple[str, str], ...] = (
    ("ws_book_events", "ts_ms"),
    ("ws_trades", "ts_ms"),
    ("ws_trades", "trade_id"),
)
...
SENTINELS_0012: tuple[tuple[str, str], ...] = (
    ("ws_obs_arrivals", "obs_time"),
    ("ws_obs_arrivals", "received_at"),
)
...
def _detect_baseline(engine: Engine, script_dir: ScriptDirectory) -> str:
    ...
    present_0010 = _present_sentinels(inspector, tables, SENTINELS_0010)
    if not present_0010:
        return "0009"
    if len(present_0010) != len(SENTINELS_0010):
        missing = set(SENTINELS_0010) - present_0010
        raise RuntimeError(
    ...
    present_0012 = _present_sentinels(inspector, tables, SENTINELS_0012)
    if not present_0012:
        return "0011"
    if len(present_0012) != len(SENTINELS_0012):
        missing = set(SENTINELS_0012) - present_0012
        raise RuntimeError(
            f"partial 0012 schema detected; present={sorted(present_0012)} missing={sorted(missing)}"
        )
    return "0012"
