...
def run(args: argparse.Namespace) -> int:
    ...
    days = event_day_inventory(coverage, tape_first=tape_first, tape_last=tape_last)
    split = freeze_split(days)
    exclusions = blind_exclusions(
        blind, scope_start=split.scope_start, scope_end=split.scope_end
    ...
    days = apply_spans(days, exclusions)
    ...
    payload |= {
        "padded_boundary_id": padded[0].boundary_id,
        "padded_gap_id": padded[0].gap_id,
