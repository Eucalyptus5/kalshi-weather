...
def test_parity_with_live_daily_max_per_member(case_key: str) -> None:
    ...
    backtest_arr = daily_high_members(times, matrix, tz, valid)

    assert np.array_equal(live_arr, backtest_arr), (
        f"divergence on {case_key}: live={live_arr.tolist()}, backtest={backtest_arr.tolist()}"
    )
